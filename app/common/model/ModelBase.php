...
class ModelBase extends Model
{
     ...
     * 把一个数值型的字段增加一个数
     */
    public function setInc($where = [], $field = true, $value = 0){

        return Db::name($this->name) -> where($where) -> setInc($field, $value);
     ...
     * 把一个数值型的减少一个数
     */
    public function setDec($where = [], $field = true, $value = 0){

        return Db::name($this->name) -> where($where) -> setDec($field, $value);
